    ...
    if ($customer_name && $customer_address && $customer_phone && !empty($cart_items)) {
        foreach ($cart_items as $item) {
            $stmt = $conn->prepare("INSERT INTO orders 
                (customer_name, customer_address, customer_phone, item_name, price, quantity, order_time, status) 
                VALUES (?, ?, ?, ?, ?, ?, NOW(), 'Pending')");
        ...
        echo "<script>
            ...
            window.location.href='thankyou.php?name=" . urlencode($customer_name) . "';
        </script>";
        exit;
    } else {
...
.order-wrapper { max-width:600px; margin:auto; }
...
.message { text-align:center; padding:10px; margin-bottom:20px; font-weight:600; border-radius:8px; }
    ...
    <div class="order-card" data-aos="fade-up">
        ...
        <form method="POST" id="orderForm">
            ...
            <div class="form-group">
                ...
                <input type="tel" id="customer_phone" name="customer_phone" placeholder="+94 77 123 4567" required pattern="\+94\s\d{2}\s\d{3}\s\d{4}">
            </div>
